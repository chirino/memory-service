...
Streaming responses can be cached in Redis so WebSocket clients can resume a
token stream after reconnecting:
...
- Exposes a random API key to the container via `MEMORY_API_KEYS`.
