...
- MongoDB transaction/outbox requirement: `MongoStore.InWriteTx` uses a real multi-document session transaction. When the outbox is enabled, MongoDB must run as a replica set or mongos; a change-stream relay assigns opaque resume-token cursors for live delivery and replay.
...
- Devcontainer lifecycle gotcha: keep `.devcontainer/devcontainer.json` on `overrideCommand: true`. The image has no long-running default command, so `overrideCommand: false` makes `wt up` report success and then immediately stops the container, causing every `wt exec` to fail.
...
- Worktree devcontainer gotcha: a linked worktree's `.git` file can point to Git metadata outside the directory mounted at `/workspaces`, so commands run through `wt exec` must not require Git metadata unless the primary checkout is also mounted.
- Kubernetes service-link gotcha: the `memory-service` Service injects a `MEMORY_SERVICE_PORT=tcp://...` environment variable into pods. Non-server commands such as the ClickHouse processor must set `enableServiceLinks: false` so this does not override the application's numeric port setting.
- ClickHouse processor deployment gotcha: keep its deployment strategy set to `Recreate`. A rolling update starts two processors with the same client ID, and the replacement cannot acquire the old pod's exclusive checkpoint lease.
- Kind local-image gotcha: the kind base must replace production digest-pinned application images with local `:latest` tags. Setting only `imagePullPolicy: IfNotPresent` does not make `kind load docker-image` satisfy a pod that still requests the production digest.
- Kind ingress ports: `deploy/kind-config.yaml` maps the Envoy node ports to host HTTP `8080` and HTTPS `8443`; kind-specific nip.io issuer, redirect, attachment, smoke-test, and displayed URLs must include `:8080`. Port-mapping changes require `task kind:reset` because `kind:up` reuses an existing node container.
