...
- Release workflow: `.github/workflows/release.yml` is a manual Java/Docker-only release job. It creates/reuses `release/vX.Y.Z`, commits Maven version changes there, publishes Maven Central artifacts, builds GHCR Docker images as per-platform digest pushes on native amd64/arm64 runners, assembles the final multi-platform tags with `docker buildx imagetools create`, tags `vX.Y.Z`, then deletes the release branch after success; reruns tolerate existing tags/releases and already-published Maven artifacts. Python and TypeScript packages are intentionally excluded.
