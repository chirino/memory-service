...
- MinIO image gotcha: upstream `minio/minio` and `minio/mc` images are no longer pullable from Docker Hub or quay.io. Compose and kustomize use the community fork images `pgsty/minio` and `pgsty/mc`, which work as drop-in replacements and include `sh` and `mc`. Testcontainers Langfuse uses `cgr.dev/chainguard/minio`.
