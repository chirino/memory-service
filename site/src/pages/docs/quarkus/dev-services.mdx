...
If the app uses Keycloak Dev Services, the extension also passes the OIDC issuer and internal discovery URL to the memory-service container. To keep development zero-configuration, it sets `MEMORY_SERVICE_OIDC_ALLOW_MISSING_AUDIENCE=true`; production deployments should always configure an audience allowlist. To exercise strict validation in Dev Services, set `memory-service.devservices.env.MEMORY_SERVICE_OIDC_ALLOW_MISSING_AUDIENCE=false` and provide `memory-service.devservices.env.MEMORY_SERVICE_OIDC_ALLOWED_AUDIENCES`.
