...
### `memory-service.response-resumer`

**Type:** `String`  
**Default:** Not set  
**Description:** Configures the response resumption backend. Valid values are `"redis"` or `"infinispan"`. When set, the memory-service container will be configured to use the corresponding dev service (Redis or Infinispan) for response resumption.

```properties
# Use Redis for response resumption
memory-service.response-resumer=redis

# Use Infinispan for response resumption
...
**Redis (for `memory-service.response-resumer=redis`):**
...
**Infinispan (for `memory-service.response-resumer=infinispan`):**
...
<Code lang="properties" code={`# Enable Redis-based response resumption
memory-service.response-resumer=redis
