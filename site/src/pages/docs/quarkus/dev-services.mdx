...
- **Infinispan** dev service (if available and `memory-service.cache.type=infinispan`) - for caching
...
**Type:** `String`  
**Default:** `"none"`  
**Description:** Configures the cache backend. Valid values are `"redis"` or `"infinispan"`. When set, the memory-service container will be configured to use the corresponding dev service (Redis or Infinispan) for caching. The response recording manager automatically uses the configured cache backend.

```properties
# Use Redis for caching (response recording manager will automatically use it)
memory-service.cache.type=redis

# Use Infinispan for caching (response recording manager will automatically use it)
...
If you're using response recording and resumption with Redis or Infinispan, add the corresponding dev service dependency:
...
<Code lang="properties" code={`# Enable Redis-based cache (response recording manager will automatically use it)
memory-service.cache.type=redis
