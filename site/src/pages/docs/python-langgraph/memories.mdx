...
1. Import `AsyncMemoryServiceStore` from `memory_service_langchain.langgraph`
...
**What changed**: `RunnableConfig` is imported from `langchain_core.runnables` and `AsyncMemoryServiceStore` is imported from `memory_service_langchain.langgraph`. `uuid` is also imported to generate unique memory keys.

**Why**: `AsyncMemoryServiceStore` is a LangGraph `BaseStore` implementation backed by Memory Service. It implements the async LangGraph store interface. `RunnableConfig` is needed so the `call_model` node can receive `user_id` and `token` from the graph's `configurable` dict.
...
`} exampleOutputLang="text" exampleOutput={`Thanks for sharing! I'll remember that about you.`}>
...
`} exampleOutputLang="text" exampleOutput={`Your name is Bob. You also mentioned you love hiking and Python programming.`}>
...
And the response should contain "bob"
`} exampleOutputLang="json" exampleOutput={`{"id":"c1234567-0000-0000-0000-000000000002","title":"What is my name?","ownerUserId":"bob","metadata":{},"createdAt":"2026-03-06T14:59:15.335675Z","updatedAt":"2026-03-06T14:59:15.494419Z","accessLevel":"owner"}`}>
...
Semantic search requires the background vector indexer to have processed the memories first. The indexer polls every 30 seconds by default; the admin endpoint `POST /admin/v1/memory-index/trigger` forces an immediate cycle.
...
- Explore the [`AsyncMemoryServiceStore`](https://github.com/chirino/memory-service/tree/main/python/langchain/memory_service_langchain/langgraph) source to understand all supported `BaseStore` operations: `aput`, `aget`, `adelete`, `asearch`, `alist_namespaces`.
