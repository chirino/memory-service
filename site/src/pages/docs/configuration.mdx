...
| <Cfg p="memory-service.vector.store.type" /> | `none`, `pgvector`, `mongo` | `none` | Vector store for semantic search |
...
| Property | Values | Default | Description |
|----------|--------|---------|-------------|
| <Cfg p="memory-service.vector.store.type" /> | `none`, `pgvector`, `mongo` | `none` | Vector store backend (defaults to matching the datastore type when `none`) |
...
When using MongoDB, the search store uses MongoDB text indexes for full-text search. Semantic search is not yet available for MongoDB.

<ConfigBlock property={`# MongoDB search store (auto-selected when datastore is mongo)
memory-service.vector.store.type=mongo`} />
