...
| Flag / Env Var | Values | Default | Description |
|----------------|--------|---------|-------------|
| <Cfg p="--encryption-dek-key" e="MEMORY_SERVICE_ENCRYPTION_DEK_KEY" /> | hex or base64 string | _(none)_ | Primary AES-256-GCM key (16/24/32 bytes). Enables at-rest encryption and signed download URLs. |
...
| `MEMORY_SERVICE_ENCRYPTION_DEK_DECRYPTION_KEYS` | comma-separated hex/base64 strings | _(none)_ | Additional old keys for decryption and token verification during key rotation |
| `MEMORY_SERVICE_ENCRYPTION_VAULT_TRANSIT_KEY` | string | _(none)_ | Vault transit key name |
| <Cfg p="--encryption-db-disabled" e="MEMORY_SERVICE_ENCRYPTION_DB_DISABLED" /> | `true`, `false` | `false` | Disable at-rest encryption for the database even when `--encryption-dek-key` is set. |
| <Cfg p="--encryption-attachments-disabled" e="MEMORY_SERVICE_ENCRYPTION_ATTACHMENTS_DISABLED" /> | `true`, `false` | `false` | Disable at-rest encryption for the attachment store even when `--encryption-dek-key` is set. |
