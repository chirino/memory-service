...
| <Cfg p="--oidc-allowed-audiences" e="MEMORY_SERVICE_OIDC_ALLOWED_AUDIENCES" />               | CSV             | _(none)_ | Optional OIDC audiences accepted by Memory Service; empty disables audience checks                           |

When OIDC is enabled, startup requires successful issuer discovery. `--oidc-allowed-clients` and `--oidc-allowed-audiences` are optional filters; when both are empty, any valid token from the configured issuer is accepted by the resolver and normal endpoint authorization still applies. If both filters are configured, both checks must pass. Allowed clients are matched against signed `azp` or `client_id` token claims. Allowed audiences are matched against the signed `aud` claim.
