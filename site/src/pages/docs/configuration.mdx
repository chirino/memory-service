...
`trusted-proxy-cidrs` affects client-IP resolution only. Leave it unset unless Memory Service
is directly behind a known TCP reverse proxy, and never use universal ranges such as
`0.0.0.0/0` or `::/0`. Forwarded host/proto values do not determine browser-facing URLs;
set `MEMORY_SERVICE_BASE_URL` for the developer frontend.
