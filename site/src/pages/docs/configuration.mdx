...
| Flag                                                   | Values                                 | Default | Description                                             |
| ------------------------------------------------------ | -------------------------------------- | ------- | ------------------------------------------------------- |
| <Cfg p="--cache-kind" e="MEMORY_SERVICE_CACHE_KIND" /> | `none`, `local`, `redis`, `infinispan` | `none`  | Cache backend for memory entries and response recording |
| `MEMORY_SERVICE_CACHE_EPOCH_TTL`                       | duration                               | `PT10M` | TTL for cached memory entries                           |
...
> **S3 incompatibility**: Attachment encryption is incompatible with S3 direct download. Set `MEMORY_SERVICE_ATTACHMENTS_S3_DIRECT_DOWNLOAD=false` when using encryption with S3.
