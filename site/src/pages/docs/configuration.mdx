...
| <Cfg p="memory-service.vector.qdrant.collection-name" /> | `memory_segments` | Qdrant collection name |
...
With <Cfg p="memory-service.vector.migrate-at-start" /> enabled (default), memory-service ensures the configured Qdrant collection exists at startup.
