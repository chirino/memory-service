...
| <Cfg p="memory-service.vector.store.type" /> | `none`, `pgvector`, `qdrant` | `none` | Optional vector store for semantic search |
...
Memory Service supports transparent encryption of all stored data — conversation titles, entry content, and file attachments — using the `memory-service.encryption.*` configuration namespace.
...
The built-in `dek` provider encrypts data with AES-256-GCM using a static key you supply:
...
> **S3 incompatibility**: S3 direct download (`memory-service.attachments.s3.direct-download=true`) is incompatible with encryption. Disable direct download when using a real encryption provider.

### Key rotation

Set `memory-service.encryption.dek.decryption-keys` to a comma-separated list of old (retired) Base64-encoded keys. New data is always encrypted with the primary `dek.key`; stored data is decrypted by trying the primary key first and then each key in `decryption-keys` in order.

```properties
memory-service.encryption.dek.key=<new-base64-key>
memory-service.encryption.dek.decryption-keys=<old-base64-key-1>,<old-base64-key-2>
```

## Vector Store Configuration

For semantic search capabilities, configure a vector store and embedding provider.

| Property | Values | Default | Description |
|----------|--------|---------|-------------|
| <Cfg p="memory-service.vector.store.type" /> | `none`, `pgvector`, `qdrant` | `none` | Optional vector store backend for semantic search |
| <Cfg p="memory-service.vector.migrate-at-start" /> | `true`, `false` | `true` | Run vector store migrations/provisioning at startup |
| <Cfg p="memory-service.search.semantic.enabled" /> | `true`, `false` | `true` | Enable semantic (vector) search |
| <Cfg p="memory-service.search.fulltext.enabled" /> | `true`, `false` | `true` | Enable full-text search |

### Embedding Provider

The embedding provider controls how text is converted to vectors for semantic search.

| Property | Values | Default | Description |
|----------|--------|---------|-------------|
| <Cfg p="memory-service.embedding.type" /> | `local`, `openai`, `none` | `local` | Embedding provider selection |

**Local** (default) — uses an in-process all-MiniLM-L6-v2 ONNX model (384 dimensions). No external API calls required.

**OpenAI** — uses the OpenAI Embeddings API for higher-quality embeddings:

| Property | Default | Description |
|----------|---------|-------------|
| <Cfg p="memory-service.embedding.openai.api-key" /> | _(required)_ | OpenAI API key |
| <Cfg p="memory-service.embedding.openai.model-name" /> | `text-embedding-3-small` | OpenAI model name |
| <Cfg p="memory-service.embedding.openai.base-url" /> | `https://api.openai.com/v1` | API base URL (for Azure OpenAI or proxies) |
| <Cfg p="memory-service.embedding.openai.dimensions" /> | _(model default)_ | Optional dimension override |

<ConfigBlock property={`# Use OpenAI embeddings
memory-service.embedding.type=openai
memory-service.embedding.openai.api-key=sk-...
memory-service.embedding.openai.model-name=text-embedding-3-small`} />

**None** — disables embedding and semantic search. Full-text search remains available.

### pgvector (PostgreSQL)

When using PostgreSQL, the vector store uses pgvector for semantic search:
...
Qdrant is a dedicated vector database that can be used as an external vector store for semantic search. When using Qdrant, the primary datastore (PostgreSQL or MongoDB) still stores all conversation data — Qdrant only stores embeddings and metadata for vector search.

| Property | Default | Description |
|----------|---------|-------------|
| <Cfg p="memory-service.vector.qdrant.host" /> | `localhost` | Qdrant server hostname |
| <Cfg p="memory-service.vector.qdrant.port" /> | `6334` | Qdrant gRPC port |
| <Cfg p="memory-service.vector.qdrant.collection-prefix" /> | `memory-service` | Prefix for derived collection name |
| <Cfg p="memory-service.vector.qdrant.collection-name" /> | _(none)_ | Optional explicit collection name override |
| <Cfg p="memory-service.vector.qdrant.api-key" /> | _(none)_ | API key for authentication |
| <Cfg p="memory-service.vector.qdrant.use-tls" /> | `false` | Enable TLS for gRPC connection |
| <Cfg p="memory-service.vector.qdrant.startup-timeout" /> | `PT30S` | Startup migration timeout for Qdrant API calls |

<ConfigBlock property={`# Qdrant vector store with local embeddings
memory-service.vector.store.type=qdrant
memory-service.vector.migrate-at-start=true
memory-service.vector.qdrant.host=localhost
memory-service.vector.qdrant.port=6334
memory-service.embedding.type=local`} />

By default, memory-service derives the collection name as `memory-service_<model>-<dimensions>`, for example `memory-service_openai-text-embedding-3-small-1536`. Set <Cfg p="memory-service.vector.qdrant.collection-name" /> to force a specific collection.

With <Cfg p="memory-service.vector.migrate-at-start" /> enabled (default), memory-service ensures the effective Qdrant collection exists at startup.

When using PostgreSQL as the primary datastore with Qdrant for vector search, full-text search is also available (using PostgreSQL's built-in text search). When using MongoDB as the primary datastore with Qdrant, MongoDB full-text search is also available.

### MongoDB

When using MongoDB, full-text search uses MongoDB text indexes. Semantic search requires an explicit vector store (`qdrant`).

<ConfigBlock property={`# MongoDB datastore with no vector store (full-text only)
memory-service.datastore.type=mongo
memory-service.vector.store.type=none`} />
