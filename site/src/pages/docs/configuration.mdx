...
`trusted-proxy-cidrs` affects client-IP resolution only. Leave it unset to ignore forwarded
client-IP headers, configure known proxy addresses for a restricted trust boundary, or use
`0.0.0.0/0,::/0` to accept forwarded client IPs from every peer in a simpler deployment.
With a universal range, direct callers can select their reported client IP, so use it only
when that behavior is acceptable or network policy ensures all traffic comes through a
controlled proxy. Forwarded host/proto values do not determine browser-facing URLs; set
...
For production attachment deployments, also review the [Security Hardening](/docs/deployment/security/#attachments) guidance on proxying downloads, active content, and durable storage.
...
| <Cfg p="--encryption-legacy-plain-read-enabled" e="MEMORY_SERVICE_ENCRYPTION_LEGACY_PLAIN_READ_ENABLED" /> | `true`, `false` | `false` | Permit headerless legacy plaintext reads when `plain` is configured as a fallback provider |

Outside testing, `plain` as the primary provider is rejected unless
`MEMORY_SERVICE_ENCRYPTION_ALLOW_PLAIN=true` is set deliberately. To migrate old headerless
plaintext values after enabling a real provider, include `plain` as a fallback provider and
set `MEMORY_SERVICE_ENCRYPTION_LEGACY_PLAIN_READ_ENABLED=true` only for the migration window.
Malformed `MSEH` envelopes always fail closed and are never treated as plaintext.
...
To rotate between providers, list the new provider first and the old provider as a fallback in `--encryption-kind`. New data is always encrypted by the first (primary) provider; existing ciphertext is decrypted by trying each provider in order. Headerless legacy plaintext is read only when `plain` is listed as a fallback and `encryption-legacy-plain-read-enabled` is set.
...
- The default URL is `http://localhost:11222`.
...
- `MEMORY_SERVICE_VECTOR_INFINISPAN_URL`, `MEMORY_SERVICE_VECTOR_INFINISPAN_USERNAME`, and `MEMORY_SERVICE_VECTOR_INFINISPAN_PASSWORD` also accept the shared `MEMORY_SERVICE_INFINISPAN_*` connection values as fallbacks.

| Flag / Env Var                                                                             | Default                  | Description                                     |
| ------------------------------------------------------------------------------------------ | ------------------------ | ----------------------------------------------- |
| <Cfg p="--vector-infinispan-url" e="MEMORY_SERVICE_VECTOR_INFINISPAN_URL" />               | `http://localhost:11222` | Infinispan REST endpoint URL                    |
...
| <Cfg p="--vector-infinispan-username" e="MEMORY_SERVICE_VECTOR_INFINISPAN_USERNAME" />     | _(none)_                 | Authentication username                         |
| <Cfg p="--vector-infinispan-password" e="MEMORY_SERVICE_VECTOR_INFINISPAN_PASSWORD" />     | _(none)_                 | Authentication password                         |
| <Cfg p="--vector-infinispan-auth-type" e="MEMORY_SERVICE_VECTOR_INFINISPAN_AUTH_TYPE" />   | `digest`                 | Auth mechanism: `basic` or `digest`             |
| <Cfg p="--vector-infinispan-use-tls" e="MEMORY_SERVICE_VECTOR_INFINISPAN_USE_TLS" />       | `false`                  | Enable TLS for the Infinispan vector connection |

<ConfigBlock
  property={`# Infinispan vector store
...
In deployments without OIDC, `Authorization: Bearer <api-key>` is also accepted as API-key compatibility auth. Prefer `X-API-Key` for new clients. When OIDC is configured, the `Authorization: Bearer` slot is reserved for OIDC JWTs, so bearer API keys and raw bearer user IDs are rejected.
...
| <Cfg p="--oidc-allow-missing-audience" e="MEMORY_SERVICE_OIDC_ALLOW_MISSING_AUDIENCE" />     | `true`, `false` | `false`  | Unsafe compatibility mode for issuer-only validation while migrating audience configuration                  |
...
| API keys without OIDC       | `X-API-Key` service principals and `Authorization: Bearer <api-key>` compatibility requests                              |
...
| <Cfg p="--developer-frontend-client-id" e="MEMORY_SERVICE_DEVELOPER_FRONTEND_CLIENT_ID" /> | string          | `developer-frontend` | OIDC client ID for the frontend                        |
| <Cfg p="--base-url" e="MEMORY_SERVICE_BASE_URL" />                                         | URL             | _(auto)_             | Base URL for OIDC redirects (auto-detected if not set) |

The developer frontend requires:
...
Access the developer frontend at `http://your-server:8080/developer/` after authentication.
