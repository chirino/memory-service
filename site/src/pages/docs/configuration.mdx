...
The event bus enables real-time Server-Sent Events (SSE) for frontend cache invalidation. Connected clients receive lightweight notifications when conversations, entries, or memberships change. Single-node deployments use the default `local` bus; multi-node deployments use `redis` or `postgres` for cross-node fan-out.

| Flag / Env Var                                                                     | Values                       | Default | Description                                               |
| ---------------------------------------------------------------------------------- | ---------------------------- | ------- | --------------------------------------------------------- |
| <Cfg p="--eventbus-kind" e="MEMORY_SERVICE_EVENTBUS_KIND" />                       | `local`, `redis`, `postgres` | `local` | Event bus backend                                         |
...
The `redis` backend uses Redis Pub/Sub for cross-node event fan-out. It reuses the same Redis connection configured for caching.
...
The `postgres` backend uses PostgreSQL LISTEN/NOTIFY for cross-node fan-out. It reuses the configured database connection.
