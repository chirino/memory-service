...
```
data: {"event":"created","kind":"entry","data":{"conversation":"<uuid>","conversation_group":"<uuid>","entry":"<uuid>","entry_channel":"history","entry_content_type":"history","entry_role":"USER"},"cursor":"<opaque-cursor>","occurredAt":"2026-09-16T12:00:00Z"}
```
...
- **`change`**: optional precise lifecycle change retained when `data` contains a full resource
- **`cursor`**: opaque durable replay position when the event came from an outbox-backed stream
- **`occurredAt`**: source commit time; replay preserves the timestamp used for live delivery
...
With `detail=full`, `data` uses the REST/OpenAPI resource shape for the stream scope:

- Agent `conversation` and `entry` events carry the agent resource shape.
- Admin `conversation`, `entry`, and `memory` events carry the `AdminConversation`, `AdminEntry`, and `AdminMemoryItem` resource shapes. Admin shapes include output-only internal fields such as `clientId` and `conversationGroupId` where applicable.
...
If the current resource no longer exists, the event keeps its summary `data`. Consumers can still process lifecycle and deletion information from that summary.
...
| `entry`        | `created`                       | `conversation`, `conversation_group`, `entry`, `entry_channel`, `entry_content_type`, `entry_role` | New entry appended. `entry_role` is included when it can be derived from entry content.                                                                                   |
| `memory`       | `created`, `updated`, `deleted` | `memory`, `memoryKind`, `revision`, `change`, lifecycle timestamps                                 | Admin-scoped durable lifecycle notification. Values, namespaces, and keys are absent from summary events.                                                                 |
...
- Browser-facing streams should keep the default history-only entry behavior, or explicitly set `entry_channels=history`, so internal `context` and `journal` entries are not exposed to end users.
...
### Initialize an admin projection from current state

The admin stream can send current resources before replay and live events:

```bash
curl -N -H "Authorization: Bearer $(get-admin-token)" \
  "http://localhost:9090/v1/admin/events?detail=full&initial_state=current"
```

`initial_state=current` requires an enabled replayable outbox. Do not combine it with `after`. The stream emits these phases in order:

1. `snapshot`, with the captured high-water cursor
2. `snapshot` resource events for current conversations, entries, and memories
3. `replay`, which covers changes after the captured cursor
4. `live`

Store the snapshot resources, then apply replay and live events with logical idempotency. The snapshot scan can observe concurrent changes, so the replay phase remains required.
...
- `entry_channels=history,context,journal` filters `entry` events by channel. If omitted, entry events default to `history`.
- `entry_content_types=history,history/lc4j` filters `entry` events by content type.
- `entry_roles=USER,AI` filters `entry` events by role when the role can be derived from entry content.

Entry filters apply only to `kind=entry` events. Non-entry events are delivered normally. User streams require an authenticated client identity to receive or enrich `context` and `journal` entry details; admin streams are available on `/v1/admin/events` and still require admin or auditor access.
...
Subscribe to user-visible history entries and trusted internal context entries:

```bash
curl -N -H "Authorization: Bearer $(get-token)" \
	  "http://localhost:9090/v1/events?kinds=entry&entry_channels=history,context"
```

Subscribe to trusted internal journal entries:

```bash
curl -N -H "Authorization: Bearer $(get-agent-token)" \
  "http://localhost:9090/v1/events?kinds=entry&entry_channels=journal"
```
...
The same concepts exist on gRPC `EventStreamService.SubscribeEvents` using `after_cursor`, `detail`, `initial_state`, `entry_channels`, `entry_content_types`, and `entry_roles`. The protobuf envelope is typed, while `EventNotification.data` contains the JSON-encoded OpenAPI resource or summary payload.
...
data: {"event":"created","kind":"entry","data":{"conversation":"e2c9a1b0-0001-4000-8000-000000000001","conversation_group":"...","entry":"...","entry_channel":"history","entry_content_type":"history","entry_role":"USER"}}
```
...
Entry-channel filtering is not an authorization boundary by itself. Context-channel entries are agent-internal by convention and can include prompt context, summaries, retrieval results, or tool payloads. Frontend-facing agent apps should expose only history-channel entry notifications unless the user interface is explicitly intended for trusted operator or processor use.

## Connection Lifecycle

**Keepalive**: a `: keepalive` comment is sent every 30 seconds.

**Phase markers**: consumers may receive `stream` / `phase` events with `{"phase":"snapshot"}`, `{"phase":"replay"}`, or `{"phase":"live"}`. Tail-only streams start with `live`. Current-state admin streams start with `snapshot`, replay changes after the boundary, and then reach `live`.
...
- Keep frontend event proxies history-only for `entry` notifications. Trusted processors can opt into `entry_channels=context`, `entry_channels=journal`, or combined sets such as `entry_channels=history,context,journal`.
- Only add durable cursor handling when you actually need replay guarantees.
- If you proxy the event stream through your agent app, let the frontend own cache invalidation behavior, but keep server-side forwarding rules explicit for internal entry channels.
