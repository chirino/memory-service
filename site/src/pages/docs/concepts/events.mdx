...
- Browser-facing streams should keep the default history-only entry behavior, or explicitly set `entry_channels=history`, so internal context-channel entries are not exposed to end users.
...
- `entry_channels=history,context` filters `entry` events by channel. If omitted, entry events default to `history`.
...
Entry filters apply only to `kind=entry` events. Non-entry events are delivered normally. The same filters are available on `/v1/admin/events`; admin streams still require admin or auditor access.
...
Subscribe to both user-visible history entries and trusted internal context entries:
...
- Keep frontend event proxies history-only for `entry` notifications. Trusted processors can opt into `entry_channels=context` or `entry_channels=history,context`.
