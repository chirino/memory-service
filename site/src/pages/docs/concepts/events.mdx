...
```
data: {"event":"created","kind":"entry","data":{"conversation":"<uuid>","conversation_group":"<uuid>","entry":"<uuid>","entry_channel":"history","entry_content_type":"history","entry_role":"USER"},"cursor":"<opaque-cursor>"}
```
...
The same concepts also exist on gRPC `EventStreamService.SubscribeEvents` using `after_cursor`, `detail`, `entry_channels`, `entry_content_types`, and `entry_roles`.
