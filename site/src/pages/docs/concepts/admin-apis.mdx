...
The `justification` query parameter is required. It is logged for audit purposes so administrators can explain why they accessed the global event stream. See [Real-Time Events](/docs/concepts/events/) for event format, kinds, and connection lifecycle details.
