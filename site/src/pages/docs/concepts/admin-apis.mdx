...
`conversationGroups.oldestSoftDeletedAt` is the oldest `updatedAt` timestamp among soft-deleted conversations. `memories.oldestSoftDeletedAt` is the oldest memory soft-delete timestamp. `outboxEvents` is `null` when the outbox feature is disabled or unsupported for the active datastore.
