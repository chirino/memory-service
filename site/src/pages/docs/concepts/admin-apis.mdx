...
Similarly, entries, memberships, forks, attachments, episodic memories, and semantic search all have admin counterparts that bypass ownership checks. Admin conversation payloads expose an `archived` boolean so clients can distinguish archived records without depending on storage timestamps.
