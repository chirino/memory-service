...
Then the response status should be 200
And the response should contain "0ef33d7b-11b1-4992-9785-681e222dbcd2"
And the response should contain "channel"
...
And the response should contain "AI"
...
`MemoryServiceProxy.list_conversations` accepts a `metadata` keyword argument with a list of filter expressions. The service combines them with `AND` and supports at most five predicates.

```python
response = await proxy.list_conversations(metadata=["status=waiting"])
