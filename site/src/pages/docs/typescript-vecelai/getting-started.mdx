...
<CodeFromFile
  ...
  lang="typescript"
  match='import { withMemoryService } from "@chirino/memory-service-vercelai";'
  before={0}
  after={0}
/>

**What changed**: The checkpoint imports `withMemoryService(...)` from `@chirino/memory-service-vercelai`.

**Why needed**: This helper manages memory/history persistence around your model call so app code stays focused on chat behavior.
