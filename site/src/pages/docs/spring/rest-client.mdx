...
`X-User-ID` is effective-user context, not a credential. Untrusted assertions are ignored, invalid credentials are rejected, and admin/system APIs retain the authenticated principal. See [Trusted Client User Identity Assertion](/docs/configuration/#trusted-client-user-identity-assertion) for API-key, OIDC, and mixed-mode configuration.
...
response.block().getConversations().forEach(conversation -> {
