...
**Local example listener docs**: Quarkus Dev Services and Spring Compose publish an HTTP container port, so their snippets must explicitly bind `0.0.0.0`, enable plaintext, disable TLS, and acknowledge non-loopback plaintext.
**Quarkus Dev Services developer frontend docs**: The integrated developer frontend is enabled only with a fixed `memory-service.devservices.port`, when the extension can provide a correct external `MEMORY_SERVICE_BASE_URL`. It defaults to a separate browser-visible `developer_frontend` API-key admin client (matching the preserved env suffix) and no OAuth login; random-port Dev Services leave it disabled.
**Quarkus Dev Services encryption docs**: The extension selects `MEMORY_SERVICE_ENCRYPTION_KIND=dek` with a well-known development-only DEK so encrypted storage and public signed attachment URLs work without setup. Direct production users to configure secure provider material.
**Quarkus Dev Services OIDC docs**: Keycloak integration defaults `MEMORY_SERVICE_OIDC_ALLOW_MISSING_AUDIENCE=true` for zero-configuration development. Document that production should use an audience allowlist and that Dev Services can restore strict validation with `memory-service.devservices.env.MEMORY_SERVICE_OIDC_ALLOW_MISSING_AUDIENCE=false` plus `MEMORY_SERVICE_OIDC_ALLOWED_AUDIENCES`.
**Spring Compose OIDC docs**: The local Spring Compose example sets `MEMORY_SERVICE_OIDC_ALLOW_MISSING_AUDIENCE=true` for zero-configuration development. Keep the site snippet aligned and direct production users to `MEMORY_SERVICE_OIDC_ALLOWED_AUDIENCES`.
**Local developer console auth docs**: Quarkus fixed-port Dev Services and the Spring Compose example use a separate browser-visible `developer_frontend` API-key client with an admin role. Warn that this is local-only, keep production on OIDC, and do not describe `X-User-ID` as part of Admin API authentication.
