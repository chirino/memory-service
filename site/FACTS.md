...
**Quarkus Dev Services encryption docs**: The extension explicitly sets `MEMORY_SERVICE_ENCRYPTION_ALLOW_PLAIN=true` for its disposable local container. Document this as a development-only plaintext opt-in and direct production users to configure a real encryption provider.
