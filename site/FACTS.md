...
**Quarkus Dev Services OIDC docs**: Keycloak integration defaults `MEMORY_SERVICE_OIDC_ALLOWED_AUDIENCES=memory-service`; document the `memory-service.devservices.env.MEMORY_SERVICE_OIDC_ALLOWED_AUDIENCES` override for other realms.
**Spring Compose OIDC docs**: The local Spring Compose example sets `MEMORY_SERVICE_OIDC_ALLOWED_AUDIENCES=memory-service`; keep the site snippet aligned with the compose file.
