...
**Standalone checkpoint dependency versions**: `java/quarkus/examples/doc-checkpoints/*/pom.xml` are standalone tutorial apps with their own Quarkus BOM import. Keep their `quarkus.platform.version` and `langchain4j.version` aligned with the repo Quarkus extension/chat example versions or generated gRPC/protobuf classes can require newer runtimes than the checkpoint resolves.
**Keycloak Dev Services config on Quarkus 3.35+**: `KeycloakDevServicesConfigBuildItem` no longer exists. For dependent dev services, use `DevServicesResultBuildItem.OwnedServiceBuilder.dependsOnConfig("quarkus.oidc.auth-server-url", ..., true)` and keep live `ConfigProvider` reads only as a fallback for explicit config or hot-reload reuse.
**Dependent Dev Services ordering**: A Dev Services supplier that uses `dependsOnConfig(...)` must return an unstarted `StartableContainer`; starting the Testcontainer inside the supplier prevents dependency callbacks from mutating env vars before launch.
**Memory Service fixed-port Dev Services**: Testcontainers `FixedHostPortGenericContainer.withFixedExposedPort(...)` adds a port binding but does not add the container port to the exposed-port set. Keep `withExposedPorts(MEMORY_SERVICE_PORT)` in the fixed-port branch or `Wait.forHttp(...).forPort(MEMORY_SERVICE_PORT)` reports the fixed host port as not exposed.
**Memory Service Dev Services storage**: The Quarkus extension memory-service container is self-contained by default: SQLite DB, local cache, SQLite vector search, local embeddings, and filesystem attachments under `/tmp/memory-service-dev`. Do not add datasource, Redis, Infinispan, Mongo, or Postgres extension dependencies just to back this dev service.
...
**Proto message rename gotcha**: After deleting or renaming protobuf messages, run the Quarkus proto module with `clean compile` (or a reactor `clean compile`) because stale generated classes under `target/generated-sources/grpc` can survive ordinary `compile` and reference descriptors that no longer exist.
