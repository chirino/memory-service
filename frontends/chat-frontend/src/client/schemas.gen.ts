...
When the entry is created, this is replaced with an href.`
        ...
        contentType: {
            type: 'string',
            description: 'MIME type of the attachment (e.g., "image/jpeg", "audio/mp3"). Required when href is provided directly.'
        },
        name: {
            type: 'string',
...
after creation. Returns 400 Bad Request if specified for non-history channels.`
        ...
        forkedAtConversationId: {
            type: 'string',
            format: 'uuid',
            description: "If the target conversation doesn't exist yet, auto-create it as a fork of this conversation. Ignored when the conversation already exists."
        },
        forkedAtEntryId: {
            type: 'string',
            format: 'uuid',
            description: 'Entry ID marking the fork point. Entries before this point are inherited; entries at and after this point are excluded. Required when forkedAtConversationId is set.'
        }
    },
    example: {
        userId: 'user_1234',
...
export const $UnindexedEntriesResponse = {
    ...
    example: {
        ...
        afterCursor: 'eyJjcmVhdGVkQXQiOiIyMDI1LTAxLTEwVDE0OjQwOjEyWiJ9'
    }
} as const;
