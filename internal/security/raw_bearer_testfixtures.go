...
func resolveRawBearer(r *TokenResolver, _ context.Context, bearerToken, clientID string, _ map[string]bool) (*Identity, error) {
	if !r.testingMode {
	...
	return newIdentity(userID, clientID, userRoles, r.rolesForClient(clientID), CredentialTesting), nil
}
