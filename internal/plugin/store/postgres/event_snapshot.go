...
func (s *PostgresStore) AdminListEventSnapshotConversations(ctx context.Context, after *registrystore.EventSnapshotConversationCursor, limit int) ([]registrystore.ConversationSummary, *registrystore.EventSnapshotConversationCursor, error) {
	if limit <= 0 {
		limit = 100
	}
...
func (s *PostgresStore) hydrateEventSnapshotConversationForks(ctx context.Context, rows []model.Conversation) error {
	if len(rows) == 0 {
		return nil
	}
...
func (s *PostgresStore) eventSnapshotConversationSummary(row *model.Conversation) (registrystore.ConversationSummary, error) {
...
func (s *PostgresStore) AdminListEventSnapshotEntries(ctx context.Context, after *registrystore.EventSnapshotEntryCursor, limit int) ([]model.Entry, *registrystore.EventSnapshotEntryCursor, error) {
	if limit <= 0 {
		limit = 100
	}
