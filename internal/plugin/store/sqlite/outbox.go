...
func (sqliteOutboxRow) TableName() string { return "outbox_events" }

func parseSQLiteOutboxCursor(cursor string) (int64, error) {
...
func formatSQLiteOutboxCursor(seq int64) string {
...
func (s *SQLiteStore) AppendOutboxEvents(ctx context.Context, events []registrystore.OutboxWrite) ([]registrystore.OutboxEvent, error) {
	...
	db := s.writeDBFor(ctx, "append outbox events")
	rows := make([]sqliteOutboxRow, 0, len(events))
	for _, event := range events {
		createdAt := event.CreatedAt
		if createdAt.IsZero() {
			createdAt = time.Now().UTC()
		}
		rows = append(rows, sqliteOutboxRow{
			Event:     event.Event,
			Kind:      event.Kind,
	...
	for _, row := range rows {
		out = append(out, registrystore.OutboxEvent{
			Cursor:    formatSQLiteOutboxCursor(row.Seq),
			Event:     row.Event,
			Kind:      row.Kind,
			Data:      []byte(row.Data),
...
func (s *SQLiteStore) ListOutboxEvents(ctx context.Context, query registrystore.OutboxQuery) (*registrystore.OutboxPage, error) {
	afterSeq, err := parseSQLiteOutboxCursor(query.AfterCursor)
	if err != nil {
		return nil, err
	}
	db := s.dbFor(ctx)
	if query.AfterCursor != "" && query.AfterCursor != "start" {
		var marker sqliteOutboxRow
...
func (s *SQLiteStore) EvictOutboxEventsBefore(ctx context.Context, before time.Time, limit int) (int64, error) {
	db := s.writeDBFor(ctx, "evict outbox events")
	if limit <= 0 {
		limit = 1000
	}
	subquery := db.Model(&sqliteOutboxRow{}).
		Select("seq").
		Where("created_at < ?", before).
		Order("seq ASC").
		Limit(limit)
	result := db.Where("seq IN (?)", subquery).Delete(&sqliteOutboxRow{})
	if result.Error != nil {
