...
func TestCheckpointValueEncryptionBindsClientID(t *testing.T) {
	store := &MongoStore{enc: newCheckpointEncryptionService(t)}
...
func TestMemoryValueEncryptionBindsMemoryID(t *testing.T) {
	store := &mongoEpisodicStore{enc: newCheckpointEncryptionService(t)}
	memoryID := uuid.New()
...
func TestConversationTitleEncryptionBindsConversationID(t *testing.T) {
	store := &MongoStore{enc: newCheckpointEncryptionService(t)}

	ciphertext, err := store.encryptConversationTitle("conversation-a", "Secret title")
	require.NoError(t, err)
	require.True(t, dataencryption.HasMagic(ciphertext))

	got, err := store.decryptConversationTitle("conversation-a", ciphertext)
	require.NoError(t, err)
	require.Equal(t, "Secret title", got)

	_, err = store.decryptConversationTitle("conversation-b", ciphertext)
	require.Error(t, err)
}
