Feature: PostgreSQL Outbox SSE Cursor Format
  ...
  I want live SSE tail events to use the PostgreSQL outbox cursor format
  So that live tail and replay share the same cursor space

  Background:
    Given I am authenticated as user "alice"

  Scenario: Live SSE tail uses Postgres outbox cursor format
    ...
    """
    {
      "channel": "HISTORY",
      "contentType": "history",
      "content": [{"role": "USER", "text": "Postgres cursor format"}]
    }
    """
