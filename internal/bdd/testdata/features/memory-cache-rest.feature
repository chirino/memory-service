Feature: Memory entries cache via REST
  As an agent
  I want context entries to be cached
  ...
  Scenario: Cache is populated when entries are added and used on subsequent reads
    ...
    And the conversation has a context entry "Hello from cache test" with epoch 1 and contentType "test.v1"
    And I record the current cache metrics
    # First read - should be cache hit (cache was populated by append)
    When I list context entries for the conversation with epoch "LATEST"
  ...
  Scenario: Cache is updated after sync
    ...
    And the conversation has a context entry "Initial content" with epoch 1 and contentType "test.v1"
    And I record the current cache metrics
  ...
  Scenario: Cache is updated after append (not just sync)
    ...
    Then the response status should be 200
    And the response should contain 1 entry
    # Append a new context entry (NOT using sync API)
    When I append an entry with content "Appended content" and channel "CONTEXT" and contentType "test.v1"
