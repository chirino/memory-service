Feature: Multi-agent memory scoping via REST
  As multiple agents
  I want context entries scoped to my client id
  ...
  Scenario: Agents see only their own context entries
    Given I am authenticated as agent with API key "test-agent-key"
    When I append an entry with content "Agent A memory" and channel "CONTEXT" and contentType "test.v1"
    Then the response status should be 201
    Given I am authenticated as agent with API key "test-agent-key-b"
    When I append an entry with content "Agent B memory" and channel "CONTEXT" and contentType "test.v1"
    Then the response status should be 201
    Given I am authenticated as agent with API key "test-agent-key"
