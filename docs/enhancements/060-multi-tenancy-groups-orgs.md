...
- **External authorization system**: For deployments with complex authorization requirements, an external system such as [SpiceDB](https://authzed.com/spicedb) (Zanzibar-style) could replace or augment the internal access control logic. SpiceDB provides relationship-based access control (ReBAC) with a dedicated schema language, making it well-suited for fine-grained permissions across orgs, teams, and resources. The memory service's `ConversationMembershipRepository` access checks would delegate to SpiceDB via its gRPC API instead of querying the local database. This option trades operational simplicity for expressive power and is worth considering if org hierarchies, cross-org delegation, or audit trails become requirements.
