...
2. Server looks up the attachment record (excluding archived records)
...
**Schema**: No new tables. Multiple rows in the existing `attachments` table can share the same `storageKey`. Add a `archivedAt` column for archives (if not already present on the attachment entity).

**On reference**: Create a new `AttachmentRecord` copying `storageKey`, `sha256`, `size`, `contentType`, `filename` from the source. All read queries and `createFromSource` lookups exclude archived records (`archivedAt IS NULL`).
...
The archive protocol solves this by keeping a record in the database at all times while the file still exists:
...
3. If this is the last non-deleted record:
     → Soft delete it (set archivedAt = now()). Commit transaction.
     → Delete the file from the FileStore.
     → Hard delete the archived record.
```
...
| Last reference, crash after archive but before file delete | Soft-deleted record still points to file | Cleanup job retries: finds archived records, deletes file, then hard deletes record |
...
**Soft-deleted records are invisible**: all read queries, `createFromSource` lookups, and access control checks filter on `archivedAt IS NULL`. No one can take a new reference to a blob that is being deleted.

**Cleanup job**: A periodic scheduled task finds archived attachment records, attempts to delete their files from the FileStore (idempotent — no error if already gone), then hard deletes the records.
...
### Step 1: Add `archivedAt` to Attachment Entity

Add a `archivedAt` timestamp column to the `attachments` table (if not already present). Update all read queries to filter on `archivedAt IS NULL`.

```sql
ALTER TABLE attachments ADD COLUMN archived_at TIMESTAMPTZ;
```
...
The source lookup must exclude archived records (`archivedAt IS NULL`) to prevent referencing a blob that is being deleted.

### Step 5: Update Deletion Logic

Implement the archive protocol described in the design section:
...
void deleteAttachment(String attachmentId) {
    ...
    if (activeCount > 1) {
        ...
        // Last reference — archive, then clean up file outside the transaction
        attachmentStore.softDelete(attachmentId);
    }
}

// Called after the transaction commits, or by the cleanup job
void cleanupArchivedAttachment(AttachmentDto att) {
...
A periodic job that retries file deletion for any archived attachment records. This handles crashes where the archive committed but the file deletion or hard delete didn't complete.

```java
@Scheduled(every = "${memory-service.attachments.cleanup-interval:5m}")
void cleanupArchivedAttachments() {
    List<AttachmentDto> archived = attachmentStore.findArchived();
    for (AttachmentDto att : archived) {
...
- Verify archived records are invisible to reads and `createFromSource`
- Verify cleanup job retries archived records
...
|------|--------|------------|
| Crash after archive, before file deletion | Soft-deleted record lingers, file remains in store | Cleanup job retries: finds archived records, deletes file, hard deletes record |
...
| Source attachment deleted during reference creation | New record points to archived blob | `createFromSource` filters on `archivedAt IS NULL`; `SELECT FOR UPDATE` serializes with concurrent deletes |
...
**Deferred**: This optimization suffers from the same cross-system consistency problem as deletion — updating the DB records and deleting duplicate files cannot be done atomically. A crash between steps 3 and 4 leaves orphaned files; a crash during step 3 could leave some records pointing to a file that gets deleted. Solving this correctly requires the same archive/locking patterns applied here, plus careful ordering to ensure no data loss. Since duplicate uploads are expected to be infrequent, this is deferred to a later enhancement.

## References

- [001-conversation-forking-design.md](../partial/001-conversation-forking-design.md) — Fork data model
